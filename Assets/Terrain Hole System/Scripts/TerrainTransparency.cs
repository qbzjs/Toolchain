...
[ExecuteInEditMode] public class TerrainTransparency : MonoBehaviour {
	...
	public void UpdateTransparencyMap() {
		...
		var slotTextures = new List<SplatPrototype>();
		for (var slotIndex = 0; slotIndex < tData.alphamapLayers; slotIndex++) {
			var slotTexture = tData.splatPrototypes[slotIndex];
			slotTextures.Add(slotTexture);
		}
		// try to find transparency-texture by exact match; else, try other name possibilities
		var transparencyTexture = slotTextures.FirstOrDefault(a=>a.texture != null && a.texture.name == "Transparent")
			?? slotTextures.FirstOrDefault(a=>a.texture != null && a.texture.name.ToLower().Contains("transparen"))
			?? slotTextures.FirstOrDefault(a=>a.texture != null && a.texture.name.ToLower().Contains("clear"))
			?? slotTextures.FirstOrDefault(a=>a.texture != null && a.texture.name.ToLower().Contains("invisible"));
		...
		if (transparencyTexture != null) {
			...
			for (var a = 0; a < tData.alphamapResolution; a++) {
				for (var b = 0; b < tData.alphamapResolution; b++) {
					float textureStrength = slotApplicationMapValues[a, b, slotTextures.IndexOf(transparencyTexture)];
