...
namespace MalbersAnimations.HAP
{
    ...
    public class IKReins : MonoBehaviour
    {
        ...
        protected Transform riderHand_L, riderHand_R, riderFinger_L, riderFinger_R;
        ...
        void OnRiderMounted()
        {
            ...
            if (RiderAnim)
            {
                ...
                riderFinger_L = riderHand_L.childCount > 1 ? riderHand_L.GetChild(1) : riderHand_L;
                riderFinger_R = riderHand_R.childCount > 1 ? riderHand_R.GetChild(1) : riderHand_R;
            }
        }
        ...
        void LateUpdate()
        {
            ...
            if (!riderHand_L || !riderHand_R) return; //There's no Reins Reference

            if (Montura.ActiveRider && Montura.ActiveRider.IsRiding)
            {
                if (freeLeftHand)
                {
                    ReinLeftHand.position = Vector3.Lerp(riderHand_L.position, riderFinger_L.position, 0.5f);     //Put it in the middle o the left hand
                }
                else
                {
                    if (freeRightHand)
                        ReinLeftHand.position = Vector3.Lerp(riderHand_R.position, riderFinger_R.position, 0.5f); //if the right hand is holding a weapon put the right rein to the Right hand
                }

                if (freeRightHand)
                {
                    ReinRightHand.position = Vector3.Lerp(riderHand_R.position, riderFinger_R.position, 0.5f); //Put it in the middle o the RIGHT hand
                }
                else
                {
                    if (freeLeftHand)
                        ReinRightHand.position = Vector3.Lerp(riderHand_L.position, riderFinger_L.position, 0.5f); //if the right hand is holding a weapon put the right rein to the Left hand
                }
            }
            else
            {
